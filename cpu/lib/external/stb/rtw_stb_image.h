...
class rtw_image {
   public:
    ...
    //  environment variable is defined, looks only in that directory for
    //  the image file. If the image was not found, searches for the
    //  specified image file first from the current directory, then in the
    //  images/ subdirectory, then the _parent's_ images/ subdirectory, and
    //  then _that_ parent, on so on, for six levels up. If the image was
    ...
    //  pixel (red, then green, then blue). Pixels are contiguous, going
    //  left to right for the width of the image, followed by the next row
    //  below, for the full height of the image.
    bool load(const std::string &filename) {
        auto n = bytes_per_pixel;    //  Dummy out parameter: original
